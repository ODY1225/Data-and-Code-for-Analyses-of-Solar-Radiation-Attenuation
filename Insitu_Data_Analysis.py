...
def CWA_StationData(istation_name):
    ...
    # Raw data 
    ## Extract valid data
    for ivar in ['SS02','VS01','CD11','ST09']: ## Required variable
    ...
    df = df[(['yyyymmddhh', 'date', 'hour','season', 'year', 
              'SS02', 'VS01', 'ST09', 'VIS', 'CD11', 'CLD', 'PP01', 
              'SWF', 'NE'])]

    # Remove rainy data
    df = df[df['PP01'] == 0]
    
    df.to_csv("D:/ODY/NTU/Research/Alishan Radiaiton Meeting/8.Thesis (B.A.)/Data_and_Code/"+istation+"_CWA_19802020.csv")
    return df
# CWA Station Data
...
for istation in ['Chiayi', 'Zhuzihu', 'Alishan']:
    ...
    idf = idf[((idf['year']>=2000)&(idf['year']<=2020))]
    Diurnalmean = idf[ivariable].groupby(idf.hour).apply(lambda g: g.mean(skipna = True))
    DiurnalQ25 = idf[ivariable].groupby(idf.hour).apply(lambda g: g.quantile(0.25))
    DiurnalQ75 = idf[ivariable].groupby(idf.hour).apply(lambda g: g.quantile(0.75))
...
condition_parameters = {'1':{'CLD':'8 ≤ CLD ≤ 10', 'VIS':'VIS < 1 km', 'color':'darkgreen'}, 
                        ...
                        'Criteria':{'CLD':[5, 5, 5], 'VIS':[5, 5, 10]},
                        'Name':{'CLD':'Cloud', 'VIS':'Visibility'}}
def PLOT_CloudFogGroupDiurnal(istation, iGroup):
    idf = CWAStation[istation]['data']
    idf_solar = idf[((idf['hour']>=8)&(idf['hour']<=17))&((idf['year']>=2000)&(idf['year']<=2020))]
    Diurnal_dict = {'1':np.nan, '2':np.nan, '3':np.nan}
    for icondition in ['1', '2', '3']:
        idf_icondition = idf_solar[(idf_solar[iGroup] == icondition)]
        count = idf_icondition.groupby(['date']).apply(lambda g: g.count())          
        iDate = count['date'][count['date']>=condition_parameters['Criteria'][iGroup][int(icondition)-1]] 
        iDatedf = pd.DataFrame()
        for d in iDate.index:
            iDatedf = pd.concat([iDatedf, idf[idf['date'] == d]])
        Diurnalmean = iDatedf[ivariable].groupby(iDatedf.hour).apply(lambda g: g.mean(skipna = True))
        DiurnalQ25 = iDatedf[ivariable].groupby(iDatedf.hour).apply(lambda g: g.quantile(0.25))
        DiurnalQ75 = iDatedf[ivariable].groupby(iDatedf.hour).apply(lambda g: g.quantile(0.75))
        DiurnalCount = int(len(iDate))
...
relationship_parameters = {'CWAvar':{'CLD':'CD11', 'VIS':'VS01'},
                           'Criteria':{'CLD':[5, 5, 5], 'VIS':[5, 5, 10]},
                           'Name':{'CLD':'Cloud', 'VIS':'Visibility'}}
def PLOT_Relationship(istation, iGroup, itime, icriteria):
    idf = CWAStation[istation]['data']
    idf_itime = idf[(idf['hour'] == itime)&((idf['year']>=2000)&(idf['year']<=2020))]
    ...
    Group_XY = Group_XY[np.array([len(Group_XY[i,1]) for i in range(len(Group_XY))]) > icriteria]
    
    Relationship = np.zeros((len(Group_XY), 5))
    for vis in range(len(Group_XY)):
        Relationship[vis,0] = Group_XY[vis,0]
        Relationship[vis,1] = np.nanmean(Group_XY[vis,1][ivariable])
        Relationship[vis,2] = np.nanmedian(Group_XY[vis,1][ivariable])
        Relationship[vis,3] = np.nanquantile(Group_XY[vis,1][ivariable], 0.25)
        Relationship[vis,4] = np.nanquantile(Group_XY[vis,1][ivariable], 0.75)
...
ivariable = 'SS02'
def PLOT_WeatherDiurnal(iweather, iseason):
    ...
    for istation in ['Chiayi', 'Zhuzihu', 'Yilan', 'Alishan']:
        idf = CWAStation[istation]['data']
        idf_iweather_iseason = idf[(idf[iweather] == 1)&(idf['season'] == iseason)&((idf['year']>=2011)&(idf['year']<=2019))]
        Diurnalmean = idf_iweather_iseason[ivariable].groupby(idf_iweather_iseason.hour).apply(lambda g: g.mean(skipna = True))
        DiurnalQ25 = idf_iweather_iseason[ivariable].groupby(idf_iweather_iseason.hour).apply(lambda g: g.quantile(0.25))
        DiurnalQ75 = idf_iweather_iseason[ivariable].groupby(idf_iweather_iseason.hour).apply(lambda g: g.quantile(0.75))
        plt.fill_between(Diurnalmean.index, DiurnalQ25, DiurnalQ75, color = CWAStation[istation]['color'], alpha = 0.15)
...
def PLOT_WinterSummerDiurnal(ivariable, istation):
    plt.figure(figsize = (5, 5), dpi = 300)
    plt.title(CWAVariable[ivariable]['long_name']+'\n(remove rainy data)', loc = 'left', weight = 'bold', fontsize = 14)
    plt.title('CWA '+ istation + ' Station\n' + 'Time: 2000-2020', loc = 'right', fontsize = 14)
    ...
    for iseason in ['DJF', 'JJA']:
        ...
        idf_iseason = idf[(idf['season'] == iseason)&((idf['year']>=2000)&(idf['year']<=2020))]
        iDates = idf_iseason.groupby(idf_iseason.date).apply(lambda g: g.count())  
        Diurnalmean = idf_iseason[ivariable].groupby(idf_iseason.hour).apply(lambda g: g.mean(skipna = True))
        DiurnalQ25 = idf_iseason[ivariable].groupby(idf_iseason.hour).apply(lambda g: g.quantile(0.25))
        DiurnalQ75 = idf_iseason[ivariable].groupby(idf_iseason.hour).apply(lambda g: g.quantile(0.75))
